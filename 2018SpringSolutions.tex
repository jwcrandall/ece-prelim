...
Is it possible, using \underline{state feedback} to change it to (a) and or (b). If yes, do it. Are the resulting systems controllable? observable? If no, explain why not.
    ...
    \begin{enumerate}
        ...
        \textbf{A.} Convert the given transfer function $\mathrm{G}(s)$ into controllable canonical form by taking the inverse Laplace transform of $\mathrm{Y}(s)$ and $\mathrm{U}(s)$. 

        $$
        \begin{aligned}    
        \mathrm{G}(s) &= \frac{\mathrm{Y}(s)}{\mathrm{U}(s)} = \frac{(s-2)(s-5)}{(s+1)(s-3)(s+4)} \left(\frac{\mathrm{X}(s)}{\mathrm{X}(s)}\right)\\
        \mathrm{Y}(s) &= s^2 \mathrm{X}(s) - 7s \mathrm{X}(s) + 10 \mathrm{X}(s)\\
        \mathrm{U}(s) &= s^3 \mathrm{X}(s) + 2s^2 \mathrm{X}(s) - 11s \mathrm{X}(s) - 12 \mathrm{X}(s) \\
        y(t) &= \ddot{x}(t) - 7 \dot{x}(t) + 10x(t) \\
        u(t) &= x^{(3)}(t) + 2\ddot{x}(t) - 11 \dot{x}(t) -12 x(t)\\
        x_1 &= x \\
        x_2 &= \dot{x} \\
        x_3 &= \ddot{x} \\
        x_c & = \left[\begin{array}{l}
        x_1 \\
        x_2 \\
        x_3
        \end{array}\right] \\
        \dot{x}_c & =\left[\begin{array}{ccc}
        0 & 1 & 0 \\
        0 & 0 & 1 \\
        12 & 11 & -1
        \end{array}\right] x_c+\left[\begin{array}{l}
        0 \\
        0 \\
        1
        \end{array}\right] u\\
        y_c &= \left[\begin{array}{lll}
        10 & -7 & 1
        \end{array}\right] x_c
        \end{aligned}
        $$

        Use full state feedback (pole placement) to change the transfer function to the desired value. Refer to Figure \ref{fig:06a_a}. Transform the transfer function  into controller canonical form.

        \begin{figure}
        \centering\fbox{\includegraphics[width=4.0in]{figs/2018s/06a_a.png}}
        \caption{ The state feedback method is used to change the position of poles of a system to any desired point in the complex plane. It can be used to make a system stable by moving the poles to the left side of the S plane if the system is unstable.}
        \label{fig:06a_a}
        \end{figure}
        ...
        G_f(s)&=\frac{G(s)}{1+G(s) H(s)}\\
        G(s) H(s) + 1 &=\frac{G(s)}{G_f(s)}\\
        & = \left(\frac{(s-2)(s-5)}{(s+1)(s-3)(s+4)}\right)\left(\frac{(s+1)(s+4)}{(s-5)}\right)\\
        G(s) H(s) & = \frac{(s-2)}{(s-3)} - 1\\
        G(s) H(s) & = \frac{1}{(s-3)}\\
        H(s) & = \left(\frac{1}{(s-3)}\right) \left(\frac{(s+1)(s-3)(s+4)}{(s-2)(s-5)}\right)\\
        & = \frac{(s+1)(s+4)}{(s-2)(s-5)}
        \end{aligned}
        $$

        By employing a plant with a transfer function $H(s)$ in a negative feedback $G(s)$ can be converted to $G_f(s)$.
        ...
        so that $K=\left[\begin{array}{lll}5 & 6 & 0\end{array}\right]$, which is called Pole Placement.
