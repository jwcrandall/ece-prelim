...
Definitions
\begin{itemize}
    \item Virtual Address: Address generated by the CPU
    \item Virtual Address Space: set of all logical addresses generated by a program
    \item Physical Address: Address actually available on memory unit
    \item Physical Address: Space 
\end{itemize}

\begin{itemize}
    \item $64$ bit virtual address $\Rightarrow$ $2^{64}$ (words or bytes) virtual address space 
    \item $41$ bit physical address
    \item $8 \text{kB} \approx 2^{13}$ page size $\Rightarrow$ 13 bit page offset
\end{itemize}



\begin{figure}
\centering\fbox{\includegraphics[width=4.0in]{figs/2018s/03_translation.png}}
\caption{Translation of virtual address to physical address}
\label{fig:03_translation}
\end{figure}

page number, page offset, TLB (translation lookaside buffer) index, TLB tag, cache index,cache tag
